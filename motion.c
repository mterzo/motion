...
static int motion_init(struct context *cnt)
{
    ...
    MOTION_LOG(NTC, TYPE_CORE, NO_ERRNO, "Opening Database ... would be config?");
    if(database_init(cnt) != 0){
        MOTION_LOG(ERR, TYPE_DB, NO_ERRNO, "Can't open database\n");
        exit(1);
    }
